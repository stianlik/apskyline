...
public class PointSourceRAM implements PointSource {
    ...
    public PointSourceRAM(PointSource data, int from, int to) {
    	if (data instanceof PointSourceRAM) {
	    	PointSourceRAM dataPtr = (PointSourceRAM) data;
		    d = dataPtr.d;
		    size = to-from;
		    this.data = new float[size*d];
		    System.arraycopy(dataPtr.data, from*d, this.data, 0, size*d);
		    nextPoint = this.data.length;
    	}
    	else {
	...
	return Arrays.copyOfRange(data, rangeStart, rangeStart+d);
    }
    ...
    @Override
    /**
     * TODO Maybe do some deallocation?
     */
    public void clear() {
    	nextPoint = 0;
    	size = 0;
