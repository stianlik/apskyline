...
public class Simulator {
	...
	public AlgorithmSummaryStatistics run(SimulatorConfiguration config) {
		...
		for (int i = -1; i < numTrials; i++) {
			...
			if (ds == DataSource.MEMORY) {
				...
				try {
					...
					Writer outCommand = new OutputStreamWriter(
							proc.getOutputStream());
					outCommand
							.write("sudo sh -c 'echo 3 >/proc/sys/vm/drop_caches'; exit\n");
					outCommand.flush();
					proc.waitFor();
					outCommand.close();
				} catch (Exception e) {
			...
			case FullPresort:
				...
				break;
			case PartialPresort:
				pointSource = DataPresorter.sortBestToFront(pointSource,
						config.getPartialPresortThreshold());
				break;
			case NoPresort:
			default:
				break;
			...
			if (timeComputeTotal >= LOGGING_TIME_THRESHOLD) {
				...
				System.out
						.format(" %18s | %13d | %4d | %12d | %9.2f ms | %9.2f ms | %7.2f ms | %9.2f ms | %9.2f ms | %11d | %10.1f | %12.2f ns | %13d | %12s |%n",
								trialString, n, d, window.size(),
								timeGenerateMS, timeSortMS, timePreprocessMS,
								timeComputeMS, timeReorgMS, alg.getCPUcost(),
								compPerTuple, avgTimePerCompNS,
								alg.getIOcost(), config.getDataGenerator()
										.getShortName());
			}
			// Calibration trial
			if (i == -1) {
		...
		stats.setTimeGenerateTotal((double) timeGenerateTotal / 1000000
				/ numTrials);
		stats.setTimeComputeTotal((double) timeComputeTotal / 1000000
				/ numTrials);
		stats.setTimePreprocessTotal((double) timePreprocessTotal / 1000000
				/ numTrials);
